...
const LoginBuyer = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    console.log("Buyer Login:", { email, password });
  };

  return (
    <div className="flex items-center justify-center min-h-screen bg-black text-white">
      <div className="w-full max-w-md p-6 bg-gray-900 rounded-lg shadow-lg">
        <h2 className="text-3xl font-bold text-blue-400 mb-6 text-center">Buyer Login</h2>
        ...
        <div className="text-center mt-3 text-gray-300">
          Don't have an account? <Link to="/signup/buyer" className="text-blue-400 hover:underline">Sign Up</Link>
        </div>
      </div>
    </div>
  );
